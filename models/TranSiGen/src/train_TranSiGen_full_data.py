...
def train_TranSiGen(args):
    ...
    # Out dir — include timestamp to avoid overwriting previous runs
    import time
    run_timestamp = time.strftime('%Y%m%d_%H%M%S')
    ablation_suffix = '' if ablation_mode == 'none' else f'_ablation_{ablation_mode}'
    if split_type == 'cells_split':
        local_out = '../results/trained_models_{}_cell_{}/{}/feature_{}_init_{}_{}{}/{}/'.format(
            cell_count, split_type, random_seed, feat_type, init_mode, train_cell_count, ablation_suffix, run_timestamp)
        print(local_out)
    else:
        local_out = '../results/trained_models_{}_cell_{}/{}/feature_{}_init_{}{}/{}/'.format(
            cell_count, split_type, random_seed, feat_type, init_mode, ablation_suffix, run_timestamp)
    os.makedirs(local_out, exist_ok=True)
    print(f'Output directory: {local_out}')
    ...
    if eval_metric:
        ...
        setup_seed(random_seed)
        _, _, test_metrics_dict_ls = model.test_model(loader=test_loader, metrics_func=['pearson', 'rmse', 'precision100'], ablation_mode=ablation_mode)
    ...
    if predict_profile:
        ...
        for name, loader in zip(['test'], [test_loader]):
            x1_array, x2_array, x1_rec_array, x2_rec_array, x2_pred_array, _, mol_id_array, cid_array, sig_array = model.predict_profile(loader=loader, ablation_mode=ablation_mode)
            ddict_data = dict()
