#!/usr/bin/env python
"""prepare.py — original_reproduction (each model on its own paper dataset).

Same data dependencies as original_ablation — each model trains on
its own paper's original dataset. Reuses the same prepare steps:
  A. extract MultiDCP tar
  B. unrar PertDiT lincs RAR
  C-H. 6 convert_<M>.py
  I. add drug_split_0..4 columns to deepce/multidcp/transigen

Usage:
    python experiments/original_reproduction/prepare.py
    python experiments/original_reproduction/prepare.py --check-only
"""
...
def main():
    ...
    args = p.parse_args()
    print("prepare.py — original_reproduction (per-model original datasets)")
